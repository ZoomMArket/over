...
var (
	procGetProcAddress       *syscall.Proc
	procLoadLibraryW         *syscall.Proc
	procVirtualProtectEx     *syscall.Proc
	procReadProcessMemory    *syscall.Proc
	procEnumProcessModules   *syscall.Proc
	procGetModuleFileNameExW *syscall.Proc
	patchAPIsOnce            sync.Once
)

func resolvePatchAPIs() {
	patchAPIsOnce.Do(func() {
		procGetProcAddress, _ = winapi.GetProc("kernel32.dll", "GetProcAddress")
		procLoadLibraryW, _ = winapi.GetProc("kernel32.dll", "LoadLibraryW")
		procVirtualProtectEx, _ = winapi.GetProc("kernel32.dll", "VirtualProtectEx")
		procReadProcessMemory, _ = winapi.GetProc("kernel32.dll", "ReadProcessMemory")
		procEnumProcessModules, _ = winapi.GetProc("psapi.dll", "EnumProcessModulesEx")
		procGetModuleFileNameExW, _ = winapi.GetProc("psapi.dll", "GetModuleFileNameExW")
	})
}
