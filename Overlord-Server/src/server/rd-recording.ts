...
function ffmpegPath(): string {
  return process.env.OVERLORD_FFMPEG_PATH?.trim() || "ffmpeg";
}
...
function targetFps(raw?: number): number {
  return requestedFps(raw) || envNumber("OVERLORD_RD_RECORD_FPS", 15, 1, 120);
}

function h264InputFps(raw?: number): number {
  const fallback = envNumber("OVERLORD_RD_RECORD_H264_FPS", 60, 1, 120);
  if (!raw || !Number.isFinite(raw)) return fallback;
  return Math.max(1, Math.min(120, Math.floor(raw)));
}

function segmentSeconds(): number {
  return envNumber("OVERLORD_RD_RECORD_SEGMENT_SECONDS", 600, 30, 3600);
}

function videoBitrate(): string {
  return process.env.OVERLORD_RD_RECORD_BITRATE?.trim() || "2500k";
}

function compactCrf(): number {
  return envNumber("OVERLORD_RD_RECORD_COMPACT_CRF", 30, 18, 40);
}

function compactPreset(): string {
  const value = process.env.OVERLORD_RD_RECORD_COMPACT_PRESET?.trim() || "veryfast";
